    ...
    if (ctype_alnum($un)) {
        ...
        //if no user exist then creat user
          if ($num_rows<1) {
               //Create new account
            if ($num_rows2>1){
              echo $_GET['callback'] . '(' . "{'status' : 'Email address is already in use'}" . ')';
            }
            else{
          ...
          if (mysqli_connect_errno())
            ...
            echo $_GET['callback'] . '(' . "{'status' : 'Failed to connect to MySQL: $err'}" . ')';
              ...
              echo $_GET['callback'] . '(' . "{'status' : 'Your username $un has been registered please check your email at $em to activate your acount.'}" . ')';
              mysqli_close($con);
